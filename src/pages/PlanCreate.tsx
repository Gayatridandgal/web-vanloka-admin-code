                ...
                <div style={{ maxWidth: 800, margin: '0 auto', width: '100%', display: 'flex', flexDirection: 'column', gap: 24 }}>
                    ...
                    <div style={{ background: 'white', borderRadius: 16, border: '1.5px solid var(--border)', padding: 24 }}>
                        ...
                        {selectedFeatures.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: '30px 0', border: '1.5px dashed var(--border)', borderRadius: 12, color: 'var(--muted)', fontSize: 13 }}>
                                No features added yet. Click 'Add Feature' to start.
                            </div>
                        ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                {selectedFeatures.map((feat, idx) => (
                                    <div key={idx} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', background: '#F8FAFC', padding: 16, borderRadius: 12, border: '1px solid var(--border)' }}>
                                        <div style={{ flex: 1 }}>
                                            <div className="form-group" style={{ marginBottom: 8 }}>
                                                <label className="form-label" style={{ fontSize: 9 }}>Feature Name</label>
                                                <div style={{ position: 'relative' }}>
                                                    <select 
                                            ...
                                            <div className="form-group" style={{ marginBottom: 0 }}>
                                                ...
                                                <div style={{ fontSize: 12, color: '#475569', minHeight: 18 }}>{feat.description || 'Select a feature to see its description...'}</div>
                                            </div>
                                        </div>
                                        <button type="button" style={{ marginTop: 22, color: '#EF4444', background: 'transparent', border: 'none', cursor: 'pointer' }} onClick={() => handleRemoveFeature(idx)}>
                                            <Trash2 size={18} />
                                        </button>
                                    </div>
                                ))}
