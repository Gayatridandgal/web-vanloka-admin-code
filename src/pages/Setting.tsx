...
export const SettingsPage: React.FC = () => {
    ...
    function saveProfile(e: React.FormEvent) {
        ...
        if (!profileDraft.fullName.trim() || !profileDraft.email.trim()) {
            setProfileMsg({ text: 'Name and email are required.', ok: false });
            return;
        }
    ...
    function savePw(e: React.FormEvent) {
        e.preventDefault();
        if (!pw.current) {
            setPwMsg({ text: 'Current password is required.', ok: false });
            return;
        }
        if (pw.next.length < 8) {
        ...
        return (
            <div className={`notif-toggle ${val ? 'notif-toggle-on' : 'notif-toggle-off'}`}>
                <div className="notif-left">
                    <div className={`notif-icon-wrap ${val ? 'notif-icon-on' : 'notif-icon-off'}`}>
                        {icon}
                    </div>
                    <div>
                    ...
                    >
                        <SectionCard title="Password & Security" icon={<Lock size={15} />}>
                            <div className="pw-header-row">
                                ...
                                {!editingPw && (
                                    <button onClick={openPwEdit} className="btn-primary">
                                        Update Password
                                    </button>
                                )}
                            </div>
                        ...
                        <div className="tfa-banner">
                            ...
                            <div>
                                ...
                                <p className="tfa-body">
                                    Add an extra security layer using Google Authenticator or Authy.
                                </p>
                    ...
                    >
                        <SectionCard title="Notification Preferences" icon={<Bell size={15} />}>
                            <div className="notif-header-row">
                                <p className="card-sub">
                                    Control how and when you receive alerts and reports.
                                </p>
                                {!editingNotifs && (
                            ...
                            {!editingNotifs ? (
                                <div className="notif-status-grid">
                                    {(
                                        ...
                                        <div
                                            key={item.key}
                                            className={`status-chip ${notifs[item.key] ? 'chip-on' : 'chip-off'}`}
                                        >
                                            <span
                                                className={
                                                    notifs[item.key]
                                            ...
                                            <span
                                                className={`chip-badge ${notifs[item.key] ? 'badge-on' : 'badge-off'}`}
                                            >
                                ...
                                <form onSubmit={saveNotifs} className="notif-edit-form">
                                    <div className="notif-toggles">
                                        <NotifToggle
                                            ...
                                            desc="Daily digest to your inbox"
                                            icon={<Mail size={16} />}
                                            field="emailAlerts"
                                        />
                                        <NotifToggle
                                            label="SMS Alerts"
                                            desc="Critical alerts to your mobile"
                                            icon={<MessageSquare size={16} />}
                                            field="smsAlerts"
                                        />
                                        <NotifToggle
